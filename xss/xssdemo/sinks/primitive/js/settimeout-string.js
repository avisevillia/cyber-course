// JS Sink: setTimeout(string)
    (function(){
      ...
      function setSolutions(){
        ...
        SinksLab.byId("puzzleSolutionText").innerHTML = `Set <b>Scheduler</b>=<b>String timers</b> and put <code>alert(1)</code> in <b>Command string</b>.`;
      }


      function takeSettimeoutString(id){ return String(SinksLab.val(id) ?? "").trim(); }
      ...
      function runDirect(){
        ...
        const v = takeSettimeoutString("direct_cmd");

        SinksLab.setText("directDebug", `used=direct_cmd | len=${v.length}`);
      ...
      function runPuzzle(){
        ...
        const knob = takeSettimeoutString("schedule");
        ...
        if(!rule){
          ...
          SinksLab.setText("puzzleDebug", `schedule=${knob} ✗ | used=(none)`);

          SinksLab.warn(`Gate check failed. Set schedule="${allow}".`, w);
        ...
        const v = takeSettimeoutString(srcId);

        SinksLab.setText("puzzleDebug", `schedule=${knob} ✓ | used=${srcId} | len=${v.length}`);

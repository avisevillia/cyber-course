// URL Sink: location.replace(...)
    (function(){
      ...
      function setSolutions(){
        ...
        SinksLab.byId("puzzleSolutionText").innerHTML = `Set <b>Policy</b>=<b>Force update</b> and put the payload in <b>Update URL</b>.`;
      }


      function takeLocationReplace(id){ return String(SinksLab.val(id) ?? "").trim(); }


      class EngineLocationReplace{

        constructor(allow){ this.allow = allow; }

        enabled(mode){ return mode === this.allow; }

        getPayload(){ return takeLocationReplace("updateUrl"); }
      ...
      function runDirect(){
        ...
        const v = takeLocationReplace("direct_update");

        SinksLab.setText("directDebug", `used=direct_update | len=${v.length}`);
      ...
      function runPuzzle(){
        ...
        const mode = takeLocationReplace("policy");

        const engine = new EngineLocationReplace(allow);


        if(!engine.enabled(mode)){

          SinksLab.setText("puzzleDebug", `policy=${mode} ✗ | used=(none)`);

          SinksLab.warn(`Engine is locked. Set policy="${allow}".`, w);
        ...
        SinksLab.setText("puzzleDebug", `policy=${mode} ✓ | used=updateUrl | len=${payload.length}`);
      ...
      function apply(prefix, value){
          try{
            ...
            // SINK:
            frame.contentWindow.location.replace(value);
          }catch(e){

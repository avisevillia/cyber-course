// URL Sink: location.assign(...)
    (function(){
      ...
      function setSolutions(){
        ...
        SinksLab.byId("puzzleSolutionText").innerHTML = `Set <b>Confirmation</b>=<b>Confirmed</b> and put the payload in <b>Continue to</b>.`;
      }


      function readLocationAssignMethod(id){ return String(SinksLab.val(id) ?? "").trim(); }
      ...
      function runDirect(){
        ...
        const v = readLocationAssignMethod("direct_next");

        if(!v){ SinksLab.warn("Direct field is empty.", w); return; }

        SinksLab.setText("directDebug", `picked=direct_next | len=${v.length}`);
      ...
      function runPuzzle(){
        ...
        const selected = readLocationAssignMethod("confirm");


        const routes = Object.create(null);

        routes[allow] = "continueTo";
        ...
        if(!srcId){
          ...
          SinksLab.setText("puzzleDebug", `confirm=${selected} ✗ | used=(none)`);

          SinksLab.warn(`No route for this option. Choose confirm="${allow}".`, w);
        ...
        const v = readLocationAssignMethod(srcId);

        SinksLab.setText("puzzleDebug", `confirm=${selected} ✓ | used=${srcId} | len=${v.length}`);
      ...
      function apply(prefix, value){
          try{
            ...
            // SINK:
            frame.contentWindow.location.assign(value);
          }catch(e){

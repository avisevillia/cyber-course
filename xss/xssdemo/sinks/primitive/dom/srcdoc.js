// DOM Sink: iframe.srcdoc
    (function(){
      ...
      function setSolutions(){
        ...
        SinksLab.byId("puzzleSolutionText").innerHTML = `Set <b>Preview type</b>=<b>HTML preview</b> and put the payload in <b>HTML body</b>.`;
      }


      function pickSrcdoc(id){ return String(SinksLab.val(id) ?? "").trim(); }
      ...
      function runDirect(){
        ...
        const v = pickSrcdoc("direct_email");

        SinksLab.setText("directDebug", `used=direct_email | len=${v.length}`);
      ...
      function runPuzzle(){
        ...
        if(!rule){
          ...
          SinksLab.setText("puzzleDebug", `preview=${knob} ✗ | used=(none)`);

          SinksLab.warn(`Gate check failed. Set preview="${allow}".`, w);
        ...
        const v = pickSrcdoc(srcId);

        SinksLab.setText("puzzleDebug", `preview=${knob} ✓ | used=${srcId} | len=${v.length}`);
      ...
      function apply(prefix, value){
          try{
            // SINK:
            SinksLab.byId(prefix + "Frame").srcdoc = value;
          }catch(e){

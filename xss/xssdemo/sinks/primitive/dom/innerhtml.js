// DOM Sink: element.innerHTML
    (function(){
      ...
      function setSolutions(){
        ...
        SinksLab.byId("puzzleSolutionText").innerHTML = `Set <b>Preview mode</b> to <b>Rich (HTML)</b> and put the payload in <b>Public reply</b>.`;
      }


      function takeInnerhtml(id){ return String(SinksLab.val(id) ?? "").trim(); }
      ...
      function runDirect(){
        ...
        const v = takeInnerhtml("direct_reply");

        SinksLab.setText("directDebug", `used=direct_reply | len=${v.length}`);
      ...
      function runPuzzle(){
        ...
        const steps = [
          ...
          (srcId)=> srcId ? takeInnerhtml(srcId) : null
        ...
        SinksLab.setText("puzzleDebug", `previewMode=${mode}${ok?" ✓":" ✗"} | used=${srcId||"(none)"} | len=${payload?payload.length:0}`);


        if(!ok){ SinksLab.warn(`Set previewMode="${allow}" to unlock.`, w); return; }
  ...
  try{
    SinksLab.byId(prefix + "Output").innerHTML = value;
  }catch(e){
